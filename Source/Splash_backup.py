...
#        event.widget.config(bg="white", bd=0, relief="flat")
   ...
   # ------------------------------------- Importing the geometry -------------------------------------<    
    ...
    def display_vtk_free_geometry(self, file_path):
        """Display geometry using PyVista VTK widget embedded in Tkinter"""
        try:
            ...
            if not hasattr(self, 'vtk_widget') or self.vtk_widget is None:
                ...
                # Create frame for VTK widget
                self.vtk_frame = tk.Frame(self.root, bg='black')
                self.vtk_frame.grid(row=0, column=1, columnspan=4, padx=1, pady=1, sticky="nsew", rowspan=9)
                
                # Create VTK render window interactor
                self.vtk_widget = vtkTkRenderWindowInteractor(self.vtk_frame, width=800, height=600)
                ...
                # Set dark background
                self.renderer.SetBackground(0.1, 0.1, 0.1)  # Dark gray
                
                # Load and process STL file
                mesh = self.load_stl_mesh_with_vtk(file_path)
                ...
                self.mesh_actor.GetProperty().SetColor(0.7, 0.9, 1.0)  # Light blue
                self.mesh_actor.GetProperty().SetOpacity(0.9)
                ...
                self.mesh_actor.GetProperty().SetEdgeColor(0.2, 0.2, 0.6)
                
                # Add actor to renderer
                self.renderer.AddActor(self.mesh_actor)
                
                # Add axes actor
                self.add_axes_to_renderer()
                
                # Set up lighting
                self.setup_vtk_lighting()
                ...
                camera.Azimuth(45)
                camera.Elevation(25)
                camera.SetParallelProjection(True)  # Orthographic view
                ...
                self.status_label.config(
                    text=f"VTK 3D: {filename} | {num_points:,} points | {num_cells:,} cells | Embedded in main window"
                )
                
                # Create control buttons
                self.create_vtk_controls()
                
            else:
                # Update existing display
                self.update_vtk_display(file_path)
            
            # Store current view state
            self.current_view = "geometry"
            
        except Exception as e:
            print(f"Error displaying VTK geometry: {e}")
            # Fallback to simple display
            self.create_fallback_info_display(file_path, f"VTK visualization error: {e}")
...
# ---------------------------------------------------------------------------<
    ...
    def create_fallback_info_display(self, file_path, message="Visualization failed"):
        ...
        try:
            ...
            # Create info display
            info_text = fSTL File Information
...
• Console information display
            ...
            print(fError creating fallback display: {e})
