...
class Splash:  
    def __init__(self, root):
        ...
        # Create a label for the "Elapsed Time" text (moved to status frame for proper layout)
        ...
        # Create a label for the timer (moved to status frame for proper layout)
        ...
        # Start updating the timer - moved to after status bar setup
        # self.update_timer()
        # ============= Time Recorder =============<
        ...
        # ----------------------
        # Create a modern ttk.Style to configure buttons
        style = ttk.Style()
        
        # Use a compatible theme for macOS
        try:
        ...
        style.map(
            "Modern.TButton",
            background=[("active", "#357abd"), ("pressed", "#2e6da4"), ("disabled", "#cccccc")],
            foreground=[("active", "white"), ("pressed", "white"), ("disabled", "#666666")],
        ...
        # Initialize VTK visualization variables  
        ...
        # ===================== FRAME-BASED LAYOUT SETUP =====================
        # Replace complex grid system with intuitive frame-based layout
        ...
        # Create main layout frames (but don't pack main_frame yet)
        self.setup_main_frames()
        
        # Setup status bar (must be after frames are created)
        self.setup_status_bar()
        
        # Start the timer now that timer labels are created
        self.update_timer()
        
        # Configure TTK styles (moved from scattered locations)
        self.setup_styles()
        
        # Setup workflow buttons now that frames exist
    ...
    def setup_main_frames(self):
        ...
        # This allows us to use pack geometry manager without conflicting with menu's grid
        self.main_frame = tk.Frame(self.root, bg="white")
        self.main_frame.pack(fill=tk.BOTH, expand=True)
        
        # Left sidebar for workflow buttons (fixed width)
        self.sidebar_frame = tk.Frame(self.main_frame, bg="lightgrey", width=200, relief=tk.RAISED, bd=1)
        ...
        self.main_container = tk.Frame(self.main_frame, bg="white")
        ...
        # Bottom area: Tabbed terminal interface (moderate height)
        self.terminal_frame = tk.Frame(self.main_container, bg="#f0f0f0", height=250, relief=tk.RAISED, bd=1)
        self.terminal_frame.pack(fill=tk.BOTH, pady=(5, 0))
        self.terminal_frame.pack_propagate(False)  # Maintain fixed height

        # Create notebook for tabs
        from tkinter import ttk
        self.terminal_notebook = ttk.Notebook(self.terminal_frame)
        self.terminal_notebook.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)
        
        # Very bottom: Status bar
        self.status_frame = tk.Frame(self.main_frame, bg="darkblue", height=30)
        self.status_frame.pack(side=tk.BOTTOM, fill=tk.X)
        self.status_frame.pack_propagate(False)
    
    def finalize_layout(self):
        """Pack the main frame after all grid components are created"""
        # Now that all grid components are created, we can pack the main frame
        # This should avoid geometry manager conflicts
        self.main_frame.pack(fill=tk.BOTH, expand=True)

    def setup_styles(self):
        """Configure TTK styles in one place"""
        style = ttk.Style()
    ...
    def setup_tabbed_terminal(self):
        ...
        # Docker Output Tab
        self.docker_frame = tk.Frame(self.terminal_notebook, bg="black")
        self.terminal_notebook.add(self.docker_frame, text="Docker Output")
        
        # Docker terminal output text widget
        self.terminal_output = scrolledtext.ScrolledText(self.docker_frame, 
                                                       height=12, 
                                                       width=100,
                                                       wrap=tk.WORD,
                                                       font=("Courier", 9),
        ...
        # Add initial message to docker output
        self.terminal_output.insert(tk.END, "Docker Terminal Output\n")
        ...
        self.terminal_frame_tab = tk.Frame(self.terminal_notebook, bg="black")
        self.terminal_notebook.add(self.terminal_frame_tab, text="Terminal")
        
        # Terminal output area (shows command history and output)
        self.interactive_output = scrolledtext.ScrolledText(self.terminal_frame_tab, 
                                                          height=10,
                                                          width=100,
                                                          wrap=tk.WORD,
                                                          font=("Courier", 9),
                                                          ...
                                                          insertbackground="white",
                                                          state=tk.DISABLED)
        self.interactive_output.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)
        
        # Command input frame
        input_frame = tk.Frame(self.terminal_frame_tab, bg="black")
        input_frame.pack(fill=tk.X, padx=2, pady=2)
        
        # Command prompt label
        prompt_label = tk.Label(input_frame, text="$", bg="black", fg="lightgreen", 
                               font=("Courier", 9, "bold"))
        prompt_label.pack(side=tk.LEFT, padx=2)
        
        # Command entry field
        self.command_entry = tk.Entry(input_frame, bg="black", fg="lightgreen", 
                                     font=("Courier", 9), insertbackground="white",
                                     highlightthickness=0, bd=0)
        self.command_entry.pack(fill=tk.X, expand=True, padx=2)
        self.command_entry.bind('<Return>', self.execute_terminal_command)
        ...
        # Add initial welcome message to interactive terminal
        self.add_terminal_message("OpenFOAM Interactive Terminal\n")
        self.add_terminal_message("=" * 40 + "\n")
        self.add_terminal_message("Type OpenFOAM commands here. Use arrow keys for command history.\n")
        self.add_terminal_message("Environment: Docker OpenFOAM\n\n")

    def add_terminal_message(self, message):
        """Add message to the interactive terminal output"""
        self.interactive_output.config(state=tk.NORMAL)
        self.interactive_output.insert(tk.END, message)
        self.interactive_output.see(tk.END)
        self.interactive_output.config(state=tk.DISABLED)
    
    def execute_terminal_command(self, event=None):
        """Execute command from interactive terminal"""
        ...
        self.add_terminal_message(f"$ {command}\n")
        
        # Clear entry
        self.command_entry.delete(0, tk.END)
        
        # Execute command (for now, simulate OpenFOAM environment)
        if command.lower() in ['ls', 'pwd', 'whoami']:
            self.add_terminal_message(f"Executing: {command}\n")
            self.add_terminal_message("(OpenFOAM environment commands will be integrated)\n\n")
        elif command.lower() == 'clear':
            self.interactive_output.config(state=tk.NORMAL)
            self.interactive_output.delete(1.0, tk.END)
            self.interactive_output.config(state=tk.DISABLED)
            self.add_terminal_message("OpenFOAM Interactive Terminal\n")
            self.add_terminal_message("=" * 40 + "\n\n")
        elif command.lower() in ['exit', 'quit']:
            self.add_terminal_message("Use application controls to exit.\n\n")
        else:
            self.add_terminal_message(f"Command '{command}' will be passed to OpenFOAM environment.\n")
            self.add_terminal_message("(Full OpenFOAM integration pending)\n\n")
    ...
    def setup_status_bar(self):
        """Set up status bar in the status frame"""
        # Initialize status label early to prevent AttributeError
        self.status_label = ttk.Label(self.status_frame, text="Ready to import geometry",
                                    font=("Helvetica", 12), background="darkblue", foreground="white")
        self.status_label.pack(side=tk.LEFT, padx=10, pady=2)
        
        # Add timer labels to the right side of status frame (instead of grid on root)
        timer_frame = tk.Frame(self.status_frame, bg="darkblue")
        timer_frame.pack(side=tk.RIGHT, padx=10, pady=2)
        
        # Create timer labels
        self.elapsed_time_label = tk.Label(timer_frame, text="Elapsed Time", 
                                         font=("Helvetica", 10), bg="darkblue", fg="lightgreen")
        self.elapsed_time_label.pack(side=tk.LEFT, padx=5)
        
        self.timer_label = tk.Label(timer_frame, text="00:00", 
                                  font=("Helvetica", 12, "bold"), bg="darkblue", fg="lightgreen")
        self.timer_label.pack(side=tk.LEFT, padx=5)

    def setup_workflow_buttons(self):
        """Set up all workflow buttons in the sidebar frame"""
        # Create a button to import a geometry file
        self.import_button = tk.Button(self.sidebar_frame, text="Import Geometry", command=self.import_geometry, 
                                     bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                     relief="solid", bd=1, padx=12, pady=10,
                                     activebackground="#bee3f8", activeforeground="#1a365d",
                                     highlightthickness=0, cursor="hand2")
        self.import_button.pack(fill=tk.X, padx=8, pady=3)
        
        # Create a button to create the mesh
        self.create_mesh_button = tk.Button(self.sidebar_frame, text="Create Mesh", command=self.create_mesh,
                                          bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                          relief="solid", bd=1, padx=12, pady=10,
                                          activebackground="#bee3f8", activeforeground="#1a365d",
                                          highlightthickness=0, cursor="hand2")
        self.create_mesh_button.pack(fill=tk.X, padx=8, pady=3)
        
        # Create a button to view the generated mesh
        self.view_mesh_button = tk.Button(self.sidebar_frame, text="View Mesh", command=self.view_mesh,
                                        bg="#f0f9ff", fg="#0c4a6e", font=("Arial", 11, "bold"), 
                                        relief="solid", bd=1, padx=12, pady=10,
                                        activebackground="#e0f2fe", activeforeground="#0c4a6e",
                                        highlightthickness=0, cursor="hand2")
        self.view_mesh_button.pack(fill=tk.X, padx=8, pady=3)
        
        # Create a button to load the case directory
        self.load_case_button = tk.Button(self.sidebar_frame, text="Load Case", command=self.load_case,
                                        bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                        relief="solid", bd=1, padx=12, pady=10,
                                        activebackground="#bee3f8", activeforeground="#1a365d",
                                        highlightthickness=0, cursor="hand2")
        self.load_case_button.pack(fill=tk.X, padx=8, pady=3)
        
        # Create a button to initialize the command execution
        self.initialize_simulation_button = tk.Button(self.sidebar_frame, text="Initialize Simulation", command=self.initialize_simulation,
                                                    bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                                    relief="solid", bd=1, padx=12, pady=10,
                                                    activebackground="#bee3f8", activeforeground="#1a365d",
                                                    highlightthickness=0, cursor="hand2")
        self.initialize_simulation_button.pack(fill=tk.X, padx=8, pady=3)
        
        # Create a button to configure the simulation settings before run
        self.configure_simulation_button = tk.Button(self.sidebar_frame, text="Configure Simulation", command=self.open_simulation_setup_popup,
                                                   bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                                   relief="solid", bd=1, padx=12, pady=10,
                                                   activebackground="#bee3f8", activeforeground="#1a365d",
                                                   highlightthickness=0, cursor="hand2")
        self.configure_simulation_button.pack(fill=tk.X, padx=8, pady=3)

        # Create a button to run simulation
        self.run_simulation_button = tk.Button(self.sidebar_frame, text="Run Simulation", command=self.run_simulation,
                                             bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                             relief="solid", bd=1, padx=12, pady=10,
                                             activebackground="#bee3f8", activeforeground="#1a365d",
                                             highlightthickness=0, cursor="hand2")
        self.run_simulation_button.pack(fill=tk.X, padx=8, pady=3)
        
        # Stop Simulation Button
        self.stop_simulation_button = tk.Button(self.sidebar_frame, text="Stop Simulation", command=self.stop_simulation,
                                              bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                              relief="solid", bd=1, padx=12, pady=10,
                                              activebackground="#bee3f8", activeforeground="#1a365d",
                                              highlightthickness=0, cursor="hand2")
        self.stop_simulation_button.pack(fill=tk.X, padx=8, pady=3)
        
        # Create a button to plot results using xmgrace (2D Plotting)
        self.plot_results_xmgrace_button = tk.Button(self.sidebar_frame, text="2D Plotting", command=self.plot_results_xmgrace,
                                                    bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                                    relief="solid", bd=1, padx=12, pady=10,
                                                    activebackground="#bee3f8", activeforeground="#1a365d",
                                                    highlightthickness=0, cursor="hand2")
        self.plot_results_xmgrace_button.pack(fill=tk.X, padx=8, pady=3)
        ...
        self.paraview_button = tk.Button(self.sidebar_frame, text="Post-processing", command=self.paraview_application,
                                       bg="#e8f4fd", fg="#1a365d", font=("Arial", 11, "bold"), 
                                       relief="solid", bd=1, padx=12, pady=10,
                                       activebackground="#bee3f8", activeforeground="#1a365d",
                                       highlightthickness=0, cursor="hand2")
        self.paraview_button.pack(fill=tk.X, padx=8, pady=3)

        # Initialize variables for removed checkbuttons
        self.reset_var = tk.BooleanVar()
        self.monitor_simulation_var = tk.BooleanVar()
        self.monitor_simulationLog_var = tk.BooleanVar()
        self.results_panel_var = tk.BooleanVar(value=True)        # Store initial profile theme values
    ...
    def add_terminal_output(self, text, also_main_console=True):
        ...
        try:
            ...
            # Terminal output widget is destroyed, ignore
            pass
        
        try:
            # Also add to main console if requested
            if also_main_console and hasattr(self, 'text_box') and self.text_box.winfo_exists():
                self.text_box.insert(tk.END, text)
                self.text_box.see(tk.END)
                self.text_box.update_idletasks()
        except tk.TclError:
            # Main console widget is destroyed, ignore
            pass
    ...
    def clear_terminal_output(self):
        ...
        # Progress bar disabled for frame-based layout
        ...
        # Status components disabled for frame-based layout
        # self.status_label_title = ttk.Label(self.root, text="")
        # status_title = "Splash v0.2"
        # self.status_label_title.grid(row=16, column=1, columnspan=1, pady=1, padx=10, sticky="nsew")
        ...
        # Status label positioning disabled - handled in status frame
    ...
    def toggle_results_panel(self):
        ...
        if self.show_first_column:
            ...
            # Timer labels are now in status frame - no need to hide/show them
...
#        event.widget.config(bg="white", bd=0, relief="flat")
    ...
    def import_geometry(self):
        ...
        if file_path:
            ...
            self.geometry_dest_path = meshing_folder  # Use the meshing folder as the destination path
   ...
   # ------------------------------------- Importing the geometry -------------------------------------<    
    ...
    def display_geometry_in_main_window(self, file_path):
        ...
        try:
            ...
            if not hasattr(self, 'main_vtk_figure') or self.main_vtk_figure is None:
                ...
                self.main_vtk_canvas = FigureCanvasTkAgg(self.main_vtk_figure, master=self.visualization_frame)
                # Place in the visualization frame using pack
                self.main_vtk_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True, padx=1, pady=1)
            ...
            # Restore text box if display fails - use visualization frame
            for widget in self.visualization_frame.winfo_children():
                widget.destroy()
            self.text_box = tk.Text(self.visualization_frame, wrap=tk.WORD, height=30, width=100)
    ...
    def toggle_console_view(self):
        ...
        if hasattr(self, 'current_view') and self.current_view == "geometry":
            ...
            # Recreate text_box in visualization frame
            for widget in self.visualization_frame.winfo_children():
                widget.destroy()
            self.text_box = tk.Text(self.visualization_frame, wrap=tk.WORD, height=30, width=100)
            self.text_box.pack(fill=tk.BOTH, expand=True, padx=1, pady=1)
    ...
    def display_simple_geometry_preview(self, file_path):
        ...
        try:
            # Background image handling removed for frame layout
    ...
    def display_vtk_free_geometry(self, file_path):
        ...
        try:
            # Clear visualization frame to make room for geometry viewer
            for widget in self.visualization_frame.winfo_children():
                widget.destroy()
            ...
            # Create the interactive viewer embedded in visualization frame
            self.create_embedded_interactive_viewer(file_path)
    ...
    def create_embedded_interactive_viewer(self, file_path):
        ...
        try:
            ...
            # Create viewer in the visualization frame instead of grid on root
            # Clear the visualization frame first
            for widget in self.visualization_frame.winfo_children():
                widget.destroy()
                
            self.viewer_frame = tk.Frame(self.visualization_frame, bg='black')
            self.viewer_frame.pack(fill=tk.BOTH, expand=True, padx=1, pady=1)
            
            # Grid configuration disabled for frame-based layout
    ...
    def setup_vtk_pipeline(self, file_path):
        ...
        try:
            ...
            self.render_window.SetSize(1200, 800)  # Larger initial size, will be updated when canvas resizes
    ...
    def create_interactive_canvas(self):
        ...
        try:
            # Create main canvas that fills the viewer_frame
            self.canvas = tk.Canvas(self.viewer_frame, bg='black')
            ...
            # Bind resize event to update VTK render window size
    ...
    def start_rendering_loop(self):
        ...
        try:
            # Update VTK render window size to match canvas after layout
            self.root.after_idle(self.update_render_window_size)
    ...
    def update_render_window_size(self):
        """Update VTK render window size to match canvas size"""
        try:
            if hasattr(self, 'canvas') and hasattr(self, 'render_window'):
                # Force canvas update to get current size
                self.canvas.update_idletasks()
                canvas_width = self.canvas.winfo_width()
                canvas_height = self.canvas.winfo_height()
                
                # Only update if canvas has reasonable size
                if canvas_width > 50 and canvas_height > 50:
                    self.render_window.SetSize(canvas_width, canvas_height)
                    print(f"🖥️ Updated VTK render window size to: {canvas_width}x{canvas_height}")
                
        except Exception as e:
            print(f"Error updating render window size: {e}")
    ...
    def render_scene(self):
        ...
        try:
            ...
            # Update canvas - handle both old and new VTK setups
            if hasattr(self, 'canvas') and self.canvas:
                # Canvas-based approach (both original and new)
                ...
                print("⚠️ No canvas available for rendering")
    ...
    def on_canvas_resize(self, event):
        ...
        try:
            if hasattr(self, 'render_window') and event.widget == self.canvas:
                ...
                if new_width > 50 and new_height > 50:
                    ...
                    self.render_window.SetSize(new_width, new_height)
                    
                    # Request a render update after a short delay to allow layout to settle
                    self.root.after(100, self.request_render)
                    
        except Exception as e:
            print(f"Error handling canvas resize: {e}")
    ...
    def create_interactive_controls(self):
        """Create control buttons for interactive viewer"""
        try:
                ...
                if hasattr(widget, 'winfo_class') and widget.winfo_class() == 'Frame':
                    ...
                    for child in widget.winfo_children():
                        if (hasattr(child, 'cget') and hasattr(child, 'winfo_class') and 
                            ...
                            try:
                                text = child.cget('text')
                                if text and '3D View Controls:' in str(text):
                                    frame_contains_vtk = True
                                    break
                            except:
                                pass
...
The main window shows available geometry information."""
    ...
    def ensure_vtk_window(self):
        """Ensure VTK visualization window exists, create if needed"""
        if hasattr(self, 'renderer') and self.renderer:
            # VTK window already exists
            print("✅ VTK window already exists")
            return
            
        try:
            # Replace text box with VTK visualization
            print("🎨 Creating VTK window for mesh viewing...")
            self.setup_mesh_only_vtk_window()
            print("✅ VTK visualization window created for mesh viewing")
        except Exception as e:
            print(f"❌ Could not create VTK window: {str(e)}")
            import traceback
            traceback.print_exc()
    
    def setup_mesh_only_vtk_window(self):
        """Set up VTK window specifically for mesh visualization (without geometry loading)"""
        print("🔧 Setting up mesh-only VTK window...")
        
        # Clear the text box and create VTK embedded viewer
        if hasattr(self, 'text_box'):
            print("📝 Removing text box...")
            self.text_box.destroy()
            
        if hasattr(self, 'text_box_scrollbar'):
            print("📜 Removing scrollbar...")
            self.text_box_scrollbar.destroy()
        
        # Use the existing embedded VTK approach but without loading geometry
        print("🎨 Creating embedded VTK viewer...")
        self.create_embedded_vtk_viewer()
        
        # Add welcome message to terminal instead
        print("🎯 VTK window ready for mesh visualization")
        print("📊 You can now load meshes and use visualization controls")
    
    def create_embedded_vtk_viewer(self):
        """Create embedded VTK viewer in the visualization frame"""
        import vtk
        
        # Create a frame to contain the VTK widget that can resize
        self.vtk_frame = tk.Frame(self.visualization_frame, bg='black', relief=tk.SUNKEN, bd=2)
        self.vtk_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Allow the frame to resize with its content
        # self.vtk_frame.pack_propagate(False)  # Removed to allow dynamic sizing
        
        try:
            # Try to use VTK with Tkinter integration (fallback approach)
            self.setup_offscreen_vtk()
            print("✅ Using offscreen VTK rendering with Tkinter")
        except Exception as e:
            print(f"❌ VTK setup failed: {e}")
            # Create a simple canvas as ultimate fallback
            self.create_simple_canvas()
    
    def setup_offscreen_vtk(self):
        """Setup VTK with offscreen rendering for better compatibility"""
        import vtk
        from PIL import Image, ImageTk
        import numpy as np
        ...
        # Create a canvas for displaying the rendered image
        self.canvas = tk.Canvas(self.vtk_frame, bg='black', highlightthickness=0)
        self.canvas.pack(fill=tk.BOTH, expand=True)
        
        # Wait for the canvas to be drawn to get actual size
        self.canvas.update_idletasks()
        ...
        # Use canvas size or fallback to reasonable defaults
        if canvas_width < 100:  # Canvas not sized yet
            canvas_width = 800
        if canvas_height < 100:
            canvas_height = 600
            
        print(f"🖥️ Setting VTK render window size to: {canvas_width}x{canvas_height}")
        
        # Set render window size to match canvas
        self.render_window.SetSize(canvas_width, canvas_height)
        ...
        # Bind canvas resize event to update VTK window size
        self.canvas.bind('<Configure>', self.on_canvas_resize)
        ...
        # Initial render - make sure it's visible
        self.render_scene()
        
        # Force a second render after a short delay to ensure visibility
        if hasattr(self, 'canvas'):
            self.canvas.after(200, self.render_scene)
        
        print("✅ Offscreen VTK viewer created with dynamic sizing and mouse interaction")
    
    def on_canvas_resize(self, event):
        """Handle canvas resize to update VTK render window size"""
        if hasattr(self, 'render_window') and event.width > 100 and event.height > 100:
            # Update render window size to match canvas
            self.render_window.SetSize(event.width, event.height)
            print(f"🖥️ Updated VTK render window size to: {event.width}x{event.height}")
    ...
    def bind_mouse_events(self):
        """Bind mouse events to canvas for VTK interaction"""
        ...
        self.canvas.bind("<Button-2>", self.on_middle_press)  # Middle mouse
        self.canvas.bind("<B2-Motion>", self.on_middle_drag)
    ...
    def on_mouse_drag(self, event):
        ...
        if self.is_rotating and hasattr(self, 'renderer'):
    ...
    def on_middle_press(self, event):
        """Handle middle mouse press for panning"""
        self.mouse_x = event.x
        self.mouse_y = event.y
    ...
    def on_middle_drag(self, event):
        ...
        if self.is_panning and hasattr(self, 'renderer'):
            dx = event.x - self.mouse_x
            dy = event.y - self.mouse_y
    ...
    def create_simple_canvas(self):
        """Create simple canvas as fallback"""
        self.canvas = tk.Canvas(self.vtk_frame, bg='black', highlightthickness=0)
        self.canvas.pack(fill=tk.BOTH, expand=True)
        
        # Add some text
        self.canvas.create_text(400, 300, text="VTK Viewer\n(Fallback Mode)", 
                               fill='white', font=('Arial', 16), anchor='center')
        
        print("✅ Simple canvas fallback created")
    ...
    def toggle_wireframe(self):
        """Toggle wireframe/surface display for mesh visualization"""
        if not hasattr(self, 'renderer') or not self.renderer:
            self.add_terminal_output("❌ VTK renderer not available")
            return
            
        # Find mesh actors
        mesh_actors = []
        for actor in self.renderer.GetActors():
            if hasattr(actor, '_is_mesh_actor'):
                mesh_actors.append(actor)
        
        if not mesh_actors:
            self.add_terminal_output("❌ No mesh loaded for wireframe toggle")
            return
            
        # Toggle representation for all mesh actors
        for actor in mesh_actors:
            current_rep = actor.GetProperty().GetRepresentation()
            if current_rep == 1:  # Wireframe
                actor.GetProperty().SetRepresentationToSurface()
                actor.GetProperty().SetOpacity(0.7)
                self.add_terminal_output("🔄 Switched to surface mode")
            else:  # Surface
                actor.GetProperty().SetRepresentationToWireframe()
                actor.GetProperty().SetOpacity(0.8)
                self.add_terminal_output("🔄 Switched to wireframe mode")
...
# -------------------------------- Mesh Construction ------------------------------>
    def create_mesh(self):
        ...
        # Ensure geometry_dest_path is set (fallback for pre-loaded geometry)
        if not hasattr(self, 'geometry_dest_path') or self.geometry_dest_path is None:
            # Use default meshing location
            script_dir = os.path.dirname(os.path.abspath(__file__))
            splash_root = os.path.dirname(script_dir)
            self.geometry_dest_path = os.path.join(splash_root, "Resources", "Geometry", "Meshing")
            
            # Create the directory if it doesn't exist
            if not os.path.exists(self.geometry_dest_path):
                os.makedirs(self.geometry_dest_path)
    ...
    def generate_cad_visual(self):
        try:
            ...
            # Text widget is destroyed, ignore
            pass
...
\n"""
    ...
    def view_mesh(self):
        """Visualize the generated OpenFOAM mesh in the VTK window"""
        # Ensure VTK window exists before trying to load mesh
        self.ensure_vtk_window()
        
        # Try to determine mesh directory from multiple sources
        mesh_dir = None
        
        # First check if we have geometry_dest_path set
        if hasattr(self, 'geometry_dest_path') and self.geometry_dest_path:
            mesh_dir = self.geometry_dest_path
        
        # If not, check if we have selected_file_path and create meshing directory path
        elif hasattr(self, 'selected_file_path') and self.selected_file_path:
            mesh_dir = os.path.join(os.path.dirname(self.selected_file_path), "Meshing")
        
        # Fallback to default meshing directory
        if not mesh_dir or not os.path.exists(mesh_dir):
            # Get the correct path to Splash directory
            splash_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            mesh_dir = os.path.join(splash_root, "Resources", "Geometry", "Meshing")
        
        # Final check if mesh directory exists
        if not os.path.exists(mesh_dir):
            tk.messagebox.showwarning("Warning", f"Mesh directory not found: {mesh_dir}\nPlease create a mesh first.")
            return
            
        self.add_terminal_output(f"\n🔍 Looking for mesh in: {mesh_dir}")
        
        # Check for different mesh file types
        vtk_dir = os.path.join(mesh_dir, "VTK")
        polyMesh_dir = os.path.join(mesh_dir, "constant", "polyMesh")
        
        # Check for standalone VTK files in the mesh directory
        standalone_vtk_files = [f for f in os.listdir(mesh_dir) if f.endswith('.vtk')]
        
        if os.path.exists(vtk_dir) and os.listdir(vtk_dir):
            # Load VTK mesh files from VTK directory
            self.add_terminal_output("📊 Found VTK files in VTK/ directory, loading mesh visualization...")
            self.load_vtk_mesh(vtk_dir)
        elif standalone_vtk_files:
            # Load standalone VTK files from mesh directory
            vtk_file = os.path.join(mesh_dir, standalone_vtk_files[0])  # Use first VTK file found
            self.add_terminal_output(f"📊 Found standalone VTK file: {standalone_vtk_files[0]}, loading mesh visualization...")
            self.load_single_vtk_file(vtk_file)
        elif os.path.exists(polyMesh_dir):
            # Convert OpenFOAM mesh to VTK and load
            self.add_terminal_output("🔄 Found OpenFOAM mesh, converting to VTK...")
            self.convert_and_load_openfoam_mesh(mesh_dir)
        else:
            # Check if there are any log files indicating mesh generation attempted
            log_files = [f for f in os.listdir(mesh_dir) if f.startswith('log.')]
            if log_files:
                self.add_terminal_output(f"❌ Mesh generation logs found but no mesh files: {log_files}")
                # Offer to create a demonstration mesh from the loaded geometry
                result = tk.messagebox.askyesno("No Mesh Found", 
                    "Mesh generation was attempted but no mesh files found.\n\n"
                    "Would you like to create a demonstration mesh from the loaded geometry?")
                if result:
                    self.create_demonstration_mesh()
            else:
                tk.messagebox.showwarning("Warning", "No mesh found. Please create a mesh first.")
    
    def create_demonstration_mesh(self):
        """Create a simple demonstration mesh from the loaded STL geometry"""
        try:
            import vtk
            
            # Check if we have a loaded STL geometry
            if not hasattr(self, 'renderer') or not self.renderer:
                tk.messagebox.showwarning("Warning", "No geometry loaded for mesh generation")
                return
            
            # Try to find the STL geometry actor
            stl_actor = None
            for actor in self.renderer.GetActors():
                if not hasattr(actor, '_is_mesh_actor'):  # This is not a mesh actor
                    stl_actor = actor
                    break
            
            if not stl_actor:
                tk.messagebox.showwarning("Warning", "No STL geometry found to mesh")
                return
            
            # Get the STL mesh data
            stl_data = stl_actor.GetMapper().GetInput()
            
            self.add_terminal_output("\n🔧 Creating demonstration mesh from STL geometry...")
            
            # Create a simple tetrahedral mesh using VTK
            delaunay = vtk.vtkDelaunay3D()
            delaunay.SetInputData(stl_data)
            ...
            if mesh.GetNumberOfPoints() == 0:
                self.add_terminal_output("❌ Failed to generate demonstration mesh")
                tk.messagebox.showerror("Error", "Failed to generate demonstration mesh")
                return
            
            # Visualize the mesh
            self.visualize_mesh_data(mesh)
            
        except Exception as e:
            self.add_terminal_output(f"❌ Error creating demonstration mesh: {str(e)}")
            tk.messagebox.showerror("Error", f"Failed to create demonstration mesh: {str(e)}")
    
    def visualize_mesh_data(self, mesh_data):
        """Visualize mesh data in the VTK window"""
        try:
            import vtk
            
            # Remove any existing mesh actors
            if hasattr(self, 'renderer') and self.renderer:
                actors_to_remove = []
                for actor in self.renderer.GetActors():
                    if hasattr(actor, '_is_mesh_actor'):
            ...
            # For large meshes, we might want to show only the surface or subsample
            num_points = mesh_data.GetNumberOfPoints()
            num_cells = mesh_data.GetNumberOfCells()
            
            self.add_terminal_output(f"Processing mesh with {num_points:,} points and {num_cells:,} cells...")
            
            # Choose visualization strategy based on mesh size
            if num_cells > 50000:
                # For large meshes, show surface or subsample
                self.add_terminal_output("Large mesh detected - showing surface representation...")
                
                # Extract surface for visualization
                surface_filter = vtk.vtkDataSetSurfaceFilter()
                surface_filter.SetInputData(mesh_data)
                surface_filter.Update()
                
                visualization_data = surface_filter.GetOutput()
                self.add_terminal_output(f"Surface extracted: {visualization_data.GetNumberOfCells():,} surface cells")
            else:
                # For smaller meshes, show all
                visualization_data = mesh_data
            
            # Create mesh visualization with wireframe
            mapper = vtk.vtkDataSetMapper()
            mapper.SetInputData(visualization_data)
            
            actor = vtk.vtkActor()
            actor.SetMapper(mapper)
            actor.GetProperty().SetColor(0.2, 0.8, 0.2)  # Green mesh
            actor.GetProperty().SetOpacity(0.8)
            actor.GetProperty().SetRepresentationToWireframe()
            actor.GetProperty().SetLineWidth(1.5)
            
            # Mark this as a mesh actor
            actor._is_mesh_actor = True
            ...
            if hasattr(self, 'renderer') and self.renderer:
                ...
                bounds = visualization_data.GetBounds()
                if bounds[0] != bounds[1]:  # Valid bounds
                    # Position camera to show mesh properly
                    center = [
                        (bounds[0] + bounds[1]) / 2,
                        (bounds[2] + bounds[3]) / 2,
                        (bounds[4] + bounds[5]) / 2
                    ...
                    camera = self.renderer.GetActiveCamera()
                    camera.SetPosition(center[0] + camera_distance, center[1] + camera_distance, center[2] + camera_distance)
                    camera.SetFocalPoint(center[0], center[1], center[2])
                    camera.SetViewUp(0, 0, 1)
                    
                    self.add_terminal_output(f"Camera positioned at bounds: {bounds}")
                else:
                    # Fallback to default camera reset
                    self.renderer.ResetCamera()
                
                # Store reference to current mesh actor for toggling
                self.current_mesh_actor = actor
                
                # Update the render window
                if hasattr(self, 'render_window') and self.render_window:
                    self.render_window.Render()
                
                # Force update of the canvas display
                if hasattr(self, 'canvas') and self.canvas:
                    self.canvas.after(100, self.render_scene)  # Delayed render to ensure canvas is ready
                    
                self.add_terminal_output(f"✅ Mesh visualization loaded: {num_points:,} points, {num_cells:,} cells")
                self.add_terminal_output("💡 Use 'Wire' button to toggle between wireframe and surface modes")
                tk.messagebox.showinfo("Success", f"Mesh visualization loaded!\n{num_points:,} points, {num_cells:,} cells\n\nTip: Use 'Wire' button to toggle mesh display mode")
            else:
                self.add_terminal_output("❌ VTK renderer not available")
                
        except Exception as e:
            self.add_terminal_output(f"❌ Error visualizing mesh: {str(e)}")
            tk.messagebox.showerror("Error", f"Failed to visualize mesh: {str(e)}")
    
    def load_vtk_mesh(self, vtk_dir):
        """Load VTK mesh files and display in VTK window"""
        try:
            import vtk
            
            # Look for VTK files
            vtk_files = [f for f in os.listdir(vtk_dir) if f.endswith(('.vtk', '.vtu'))]
            
            if not vtk_files:
                self.add_terminal_output(f"❌ No VTK mesh files found in {vtk_dir}")
                tk.messagebox.showwarning("Warning", "No VTK mesh files found")
                return
                
            # Load the first VTK file found (usually the internal mesh)
            vtk_file = os.path.join(vtk_dir, vtk_files[0])
            self.add_terminal_output(f"📊 Loading mesh from: {os.path.basename(vtk_file)}")
            
            # Clear current mesh actors (keep geometry)
            if hasattr(self, 'renderer') and self.renderer:
                # Remove all mesh actors except STL geometry
                actors_to_remove = []
                for actor in self.renderer.GetActors():
                    # Check if this is a mesh actor (not the original STL)
                    if hasattr(actor, '_is_mesh_actor'):
                        actors_to_remove.append(actor)
            ...
            if mesh.GetNumberOfPoints() == 0:
                self.add_terminal_output("❌ Mesh file appears to be empty")
                tk.messagebox.showwarning("Warning", "Mesh file appears to be empty")
                return
            
            # Use the shared visualization method
            self.visualize_mesh_data(mesh)
                
        except Exception as e:
            self.add_terminal_output(f"❌ Error loading VTK mesh: {str(e)}")
            tk.messagebox.showerror("Error", f"Failed to load mesh: {str(e)}")
    
    def load_single_vtk_file(self, vtk_file):
        """Load a single VTK mesh file and display in VTK window"""
        try:
            import vtk
            ...
            if mesh.GetNumberOfPoints() == 0:
                self.add_terminal_output("❌ VTK file contains no mesh data")
                tk.messagebox.showwarning("Warning", "VTK file contains no mesh data")
                return
            ...
            self.add_terminal_output(f"❌ Error loading VTK file: {str(e)}")
            tk.messagebox.showerror("Error", f"Failed to load VTK file: {str(e)}")
    
    def convert_and_load_openfoam_mesh(self, case_dir):
        """Convert OpenFOAM mesh to VTK format and load it"""
        try:
            self.add_terminal_output("\n🔄 Converting OpenFOAM mesh to VTK format...")
            
            # Use Docker to convert mesh to VTK
            docker_cmd = [
                "docker", "run", "--rm", 
                "-v", f"{case_dir}:/case", 
                "-w", "/case",
                "fsys/openfoam4-paraview50-cfmesh:latest",
                "bash", "-c", 
                ". /opt/openfoam4/etc/bashrc && foamToVTK"
            ]
            ...
            if result.returncode == 0:
                self.add_terminal_output("✅ Mesh converted to VTK format")
                
                # Now load the VTK files
                vtk_dir = os.path.join(case_dir, "VTK")
                if os.path.exists(vtk_dir):
                    self.load_vtk_mesh(vtk_dir)
                else:
                    self.add_terminal_output("❌ VTK directory not created")
            else:
                self.add_terminal_output(f"❌ Mesh conversion failed: {result.stderr}")
                tk.messagebox.showerror("Error", "Failed to convert mesh to VTK format")
                
        except Exception as e:
            self.add_terminal_output(f"❌ Error converting mesh: {str(e)}")
            tk.messagebox.showerror("Error", f"Failed to convert mesh: {str(e)}")
...
# -------------------------------- Plot results ------------------------------  
    ...
    def setup_ui(self):
        ...
        self.text_box = tk.Text(self.visualization_frame, wrap=tk.WORD, height=30, width=100)
        self.text_box.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)  # Use pack for frame-based layout
        self.text_box.configure(foreground="lightblue", background="black", font=("courier", 10, "bold"))
        ...
        self.text_box_scrollbar = tk.Scrollbar(self.visualization_frame, command=self.text_box.yview)
